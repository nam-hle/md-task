...
md-task add "desc" --depends-on 3,5   # Add with dependencies
...
md-task view <id>                     # View single task details
...
md-task move <id> done                # Mark done (replaces old `done` command)
md-task move <id> in-progress         # Mark in-progress (replaces old `start` command)
md-task move <id> done --force        # Bypass transition validation
